...
>
> **Jembatan testing (bukan T-10 selesai):** `lib/routes/admin_seed_screen.dart` sudah dibuat — layar tersembunyi (tile sementara di menu Profil) yang memanggil `seedPrototipe()`: 7 lapangan berkoordinat terverifikasi, termasuk 3 mitra dimiliki akun yang sedang login. Tujuannya supaya alur utama (cari lapangan → detail → rating → cari rekan → reservasi) bisa dicoba di device/emulator nyata minggu ini, **tanpa menunggu 30 lapangan lengkap.** Tetap butuh T-06 (deploy rules+index) lebih dulu — tanpa itu sebagian besar stream masih akan error `failed-precondition` di Firestore asli. Tile dan file ini WAJIB dihapus setelah `AdminSeedScreen` sungguhan (T-10, 30 lapangan) menggantikannya — jangan sampai ikut ke APK yang dibagikan ke responden SUS (T-31).
>
> **Testing manual pertama (20 Agustus 2026):** dijalankan di emulator Android sungguhan lewat jembatan di atas. Home, Detail Lapangan (badge T-36, rating, favorit ♡), pencarian, dan alur reservasi dicoba langsung. Ketemu satu bug nyata — dicatat di `PRD.md` "Koreksi teknis" #8: lapangan yang tutup jam `00:00` (tengah malam) tidak pernah punya jam tersedia di form Ajukan Reservasi (L-10), karena jam tutup dibaca literal sebagai `0` alih-alih tengah malam. Sudah diperbaiki dan diverifikasi ulang (submit reservasi berhasil, notifikasi masuk ke mitra). Ini sinyal bagus untuk mulai T-30 (Black Box) begitu T-06/T-10 beres — kemungkinan ada bug serupa lain yang baru ketemu lewat testing sungguhan, bukan cuma `flutter analyze`.
