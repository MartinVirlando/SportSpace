...
> **Catatan status per 21 Agustus 2026:** Seluruh alur reservasi + dashboard mitra (T-23–T-27), keputusan T-40 (Opsi A, tanpa Firebase Storage — lihat PRD §12b), dan seluruh empat tugas v1.1 (T-35 favorit, T-36 badge, T-37 olahraga favorit + lokasi default, T-38 statistik profil) sudah selesai. **Sprint 3 dan 4 lengkap secara kode, dan T-10 (seeding) sudah benar-benar dijalankan di project Firestore produksi.** Sisa pekerjaan: T-00a–T-00e, T-06 (lihat catatan di bawah — perlu verifikasi/penyelesaian manual sebelum Sprint 5), lalu T-28 dan seterusnya (pengujian, SUS, penulisan Bab 4–5).
>
> T-06 SENGAJA belum ditandai meski rules+index sudah di-deploy — status "Enabled" 11 index hanya bisa dipastikan lewat cek manual di Firebase Console → Firestore → Indexes, bukan dari kode. **Petunjuk kuat sudah Enabled:** sepanjang testing 20–21 Agustus, seluruh query yang butuh composite index (stream aktivitas, notifikasi, booking, ulasan, dsb.) jalan tanpa error `failed-precondition` — tapi tetap cek manual sebelum mencentang, sesuai prinsip dokumen ini. T-00a–T-00e juga belum ditandai — tugas persiapan tim (instalasi, survei manual) yang tidak bisa diverifikasi lewat kode.
>
